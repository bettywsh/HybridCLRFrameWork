...
public class PrefabVarEditor : BaseEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Create CSharp Panel"))
        {
             var fullFilePath =
                EditorUtility.SaveFilePanel($"Please select a folder to create", Application.dataPath + "/Hotfix/UI/Panel", mPrefabVar.gameObject.name, "cs");
            //fullFilePath = fullFilePath + ".bytes";
            if (fullFilePath.StartsWith(".."))
            {
                EditorUtility.DisplayDialog("错误", "当前指定路径并不在任何一个CS根目录下!!!", "知道了");
                return;
            }
       
            string templua = AssetDatabase.LoadAssetAtPath<TextAsset>("Assets/Editor/UI/TempPanel.bytes").text;
            ...
            EditorUtility.DisplayDialog("成功", "创建CSharp成功!!!", "知道了");
        }
